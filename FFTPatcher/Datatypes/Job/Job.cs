...
namespace FFTPatcher.Datatypes.Job
{
    ...
    public class AllJobs
    {
        ...
        static AllJobs()
        {
            ...
            Names = new List<string>(0xA9);
            for (int i = 0; i < 0xA9; i++)
            {
                Names.Add(doc.SelectSingleNode(string.Format("//Job[@offset='{0}']/@name", i.ToString("X2"))).InnerText);
            }
        }

        public AllJobs(SubArray<byte> bytes)
        {
            Jobs = new List<Job>(0xA9);
            for (int i = 0; i < 0xA9; i++)
            {
                Jobs.Add(new Job((byte)i, Names[i], new SubArray<byte>(bytes, 49 * i, 49 * i + 48)));
            }
        }
