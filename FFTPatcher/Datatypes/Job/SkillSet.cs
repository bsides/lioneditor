...
namespace FFTPatcher.Datatypes.Job
{
    ...
    {
        private static List<SkillSet> skillSets;
        public static List<SkillSet> DummySkillSets
        {
            get
            {
                if (skillSets == null)
    ...
    public class AllSkillSets
    {
        ...
        public AllSkillSets(SubArray<byte> bytes)
        {
            ...
            for (int i = 0; i < 176; i++)
            {
                SkillSets.Add(new SkillSet((byte)i, new SubArray<byte>(bytes, 25 * i, 25 * i + 24)));
            }
            for (int i = 0xE0; i <= 0xE2; i++)
            {
                SkillSets.Add(new SkillSet((byte)i, new SubArray<byte>(bytes, 25 * (i - 0xE0 + 176), 25 * (i - 0xE0 + 176) + 24)));
            }
        }
