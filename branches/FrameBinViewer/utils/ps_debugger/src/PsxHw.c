...
int _agemo_gpu_upload_x, _agemo_gpu_upload_y, _agemo_gpu_upload_w, _agemo_gpu_upload_h;
int _agemo_gpu_last_status = 0;
...
#endif
			GPU_writeData(value); 
			if(_agemo_gpu_last_status == 0 && value == 0x01000000)
			{
				_agemo_gpu_last_status = 1;	//识别出，已经进入第1步 Flush cache
				return;
			}
			
			if(_agemo_gpu_last_status == 1 && value == 0xA0000000)
			{
				_agemo_gpu_last_status = 2;	//识别出，已经进入第2步 uploading
				return;
			}

			if(_agemo_gpu_last_status == 2)
			{
				_agemo_gpu_upload_x = value & 0xFFFF;
				_agemo_gpu_upload_y = value >> 16;
				_agemo_gpu_last_status = 3;	//记下识别出 Send destination X and Y
				return;
			}

			if(_agemo_gpu_last_status == 3)
			{
				_agemo_gpu_upload_w = value & 0xFFFF;
				_agemo_gpu_upload_h = value >> 16;
				_agemo_gpu_last_status = 0;	//记下识别出 Send destination W and H
				return;
			}
			return;
			}
