...
{	
	...
	public class LoopControl
	{
        ...
        private static Form form;   // The form connected to this controller
        ...
        static public void SetAction(Form p_form, LoopAction p_la)
        {
            form = p_form;
            loopAction = p_la;

            // Mono running on Windows doesn't fire Application.Idle for Mono 2.2.
            // Therefore, start processing look in OnShown event.
            if (IsRunningOnMono() && !IsLinux())
            {
        ...
        /// <summary>
        /// Only called when Mono is run on Windows.
        /// This will start the whole loop of the application once the form is shown.
        /// </summary>
        ...
        static void form_Shown(object sender, EventArgs e)
        {
            ...
            while (true)
            {
                if (PeekMessage(out msg, new IntPtr(0), 0, 0, (int)(PeekMessageOption.PM_NOREMOVE)))
                {
