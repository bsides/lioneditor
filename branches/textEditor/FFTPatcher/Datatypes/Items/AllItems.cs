...
namespace FFTPatcher.Datatypes
{
    ...
    public class AllItems : PatchableFile, IXmlDigest
    {
        ...
        public override IList<PatchedByteArray> GetPatches( Context context )
        {
            ...
            {
                result.Add( new PatchedByteArray( PatcherLib.Iso.PsxIso.SCUS_942_21, 0x536B8, first ) );
            }
            else if ( context == Context.US_PSP )
            {
                var second = ToSecondByteArray();
                result.Add( new PatchedByteArray( PatcherLib.Iso.PspIso.Files.PSP_GAME.SYSDIR.BOOT_BIN, 0x3252DC, first ) );
                result.Add( new PatchedByteArray( PatcherLib.Iso.PspIso.Files.PSP_GAME.SYSDIR.BOOT_BIN, 0x256E00, second ) );
                result.Add( new PatchedByteArray( PatcherLib.Iso.PspIso.Files.PSP_GAME.SYSDIR.EBOOT_BIN, 0x3252DC, first ) );
                result.Add( new PatchedByteArray( PatcherLib.Iso.PspIso.Files.PSP_GAME.SYSDIR.EBOOT_BIN, 0x256E00, second ) );
            }
