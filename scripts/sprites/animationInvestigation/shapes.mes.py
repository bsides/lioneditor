...
def innerGetFrames(bytes, imageIn, secondHalf, thirdHalf, start, startingFrameNumber=0):
	...
	for frameNumber in xrange(startingFrameNumber,numberOfFrames+startingFrameNumber):
		...
		for tileNumber in xrange(0,numberOfTile+1):
			...
			rects.append((imageIn, tileX, tileY, width, height, im, x, y, mirror, mirror2))
		rects.reverse()
		for r in rects:
...
def processSequence(bytes,num,frames):
	...
	#if num in (6,48,102): print 'seq %s %s' % (num,bytes)
	...
	for j in xrange(len(frames)):
		cmd = [r'D:\software\ImageMagick-6.4.9-Q16\convert.exe','-dispose','Previous']
		for p in sequence:
			delay = p[1]
			if delay == 0xFF:
...
def readSequence(bytes, frames, palette):
	...
	#last = animationStart
	#for i in xrange(animationStart,len(bytes),2):
	#	if bytes[i] in (0xff,0xfe):
	#		processSequence(bytes[last:i+2],last)
	#		last = i+2
	#return
