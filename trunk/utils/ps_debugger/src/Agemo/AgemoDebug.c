...
void Agemo_OnReadMem(u32 uMem, int nBytes)
{
	...
	if (uMem <= agemo_r_ram_2 )
	{
		...
		{
			__agemo_pause_cpu(1);
			AgemoTrace("CPU Break, mem read at %08X, %d bytes", uMem, nBytes);
		}
	}
/*
...
void Agemo_OnWriteMem(u32 uMem, int nBytes)
{
	uMem = uMem & 0x00FFFFFF;
	...
	if (uMem <= agemo_w_ram_2 )
	{
		...
		{
			__agemo_pause_cpu(1);
			AgemoTrace("CPU Break, Mem Write at %08X, %d bytes", uMem, nBytes);
		}
	}

	if( uMem <= agemo_memval_addr && uMem + nBytes > agemo_memval_addr)
	{
		if(psxM[uMem-0x80000000] == agemo_memval_val)
		{
			__agemo_pause_cpu(1);
...
void UI_OnChkPauseOnMemVal()
{
	...
	if (1 == n)
	{
		...
		AgemoTrace("wait for pause on mem address %08X = %02x",  agemo_memval_addr, agemo_memval_val);
...
{
	char pBuf[1024];
	sprintf(pBuf, "%d", agemo_ops_total_exec);
