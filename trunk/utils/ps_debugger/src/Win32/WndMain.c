...
/*
	...
	Agemo_CreateDbgWndThread(hInstance);
//	SysPrintf("%s\n", lpCmdLine);
...
void CreateMainWindow(int nCmdShow) {
	...
	gApp.hWnd = hWnd;
	hAgemoMainWnd = hWnd;
	//Get pseudo handle, it always got -1, which means itself
	//hKageMainThread = GetCurrentThread();

	DuplicateHandle(GetCurrentProcess(), GetCurrentThread(), 
		GetCurrentProcess(), &hAgemoMainWnd, 
...
void SysClose() {
	...
	__agemo_log_flush();
	if (instructionsLog != NULL) fclose(instructionsLog);
